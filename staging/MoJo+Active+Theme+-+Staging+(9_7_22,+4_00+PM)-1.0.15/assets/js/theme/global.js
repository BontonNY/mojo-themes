...
export default class Global extends PageManager {
	onReady() {
    	...
    	if($('.get_navPages-1').length) {
			...
			getmegaNav.getPage('/global-navigation-menu/').then(html => {
				var megaNav1 = $(html).find('#navPages-1').html();
				$('#global-nav #navPages-1').html(megaNav1);
				var megaNav2 = $(html).find('#navPages-2').html();
				$('#global-nav #navPages-2').html(megaNav2);
				var megaNav3 = $(html).find('#navPages-3').html();
				$('#global-nav #navPages-3').html(megaNav3);
				var megaNav4 = $(html).find('#navPages-4').html();
				$('#global-nav #navPages-4').html(megaNav4);
				var megaNav5 = $(html).find('#navPages-5').html();
				$('#global-nav #navPages-5').html(megaNav5);
				var megaNav6 = $(html).find('#navPages-6').html();
				$('#global-nav #navPages-6').html(megaNav6);
				var megaNav7 = $(html).find('#navPages-7').html();
				$('#global-nav #navPages-7').html(megaNav7);
				var megaNav8 = $(html).find('#navPages-8').html();
				$('#global-nav #navPages-8').html(megaNav8);
				var megaNav9 = $(html).find('#navPages-9').html();
				$('#global-nav #navPages-9').html(megaNav9);
				var megaNav10 = $(html).find('#navPages-10').html();
...
*/
	...
	onXlDown(media){
		if (media.matches) {		  
			...
			$('#mobile-nav .level-1 .has-sub-menu').each(function(){
				let level1Href = $(this).find('a.nav-link').attr('href');
				let level1Text = $(this).find('a.nav-link').text();
				$(this).find('a.nav-link.menu-bold').attr('href', "javascript:void(0)");
				$(this).find('ul').prepend(`<li><a class="menu-bold shop-all-link" href="${level1Href}">Shop All ${level1Text}</a></li>`);
