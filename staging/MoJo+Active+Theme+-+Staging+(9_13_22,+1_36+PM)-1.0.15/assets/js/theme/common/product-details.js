...
export default class ProductDetails {
  ...
  showSelectedOptionValue() {
    ...
    $('[data-product-option-change] .form-field').each(function(){
      let id = $(this).find('.form-option').first().attr('for');
      if($("#"+id).is('input')){
        $("#"+id).trigger('click');
      } 
    });
  ...
  imageSwatchesPDP() {
    ...
    fetch(`https://bc.mojoactive.com/products?id=${this.context.product.id}&include=variants`, {
        method: "GET",
        headers: {
            "X-STOREIDENTIFIER": "e1db8146-68bf-4932-9402-0809f1cb9b2d",
        },
    })
