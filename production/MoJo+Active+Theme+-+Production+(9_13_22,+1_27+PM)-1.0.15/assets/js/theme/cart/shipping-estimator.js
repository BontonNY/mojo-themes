...
export default class ShippingEstimator {
    ...
    bindUPSRates() {
        ...
        $('body').on('click', UPSRateToggle, (event) => {
            ...
            $estimatorFormUps.toggleClass('d-none');
            $estimatorFormDefault.toggleClass('d-none');
        });
    }
    ...
    bindEstimatorEvents() {
        ...
        $('.shipping-estimate-show').on('click', event => {
            ...
            $estimatorContainer.removeClass('d-none');
            $('.shipping-estimate-hide').show();
            if ($('.shipping-estimate-hide').length > 0) {
                $('.shipping-estimate-hide').focus();
            } else {
                $('.form.estimator-form').find('select, input, button').first().focus();
            }
            $('.form.estimator-form').find('select, input, button').attr('aria-hidden', 'false').attr('tabindex', '0');
        });
        ...
        $('.shipping-estimate-hide').on('click', event => {
            ...
            $estimatorContainer.addClass('d-none');
            $('.shipping-estimate-show').show();
            $('.shipping-estimate-show').focus();
            $('.shipping-estimate-hide').hide();
            $('.form.estimator-form').find('select, input, button').attr('aria-hidden', 'true').attr('tabindex', '-1');
        });
    }
}
